...
int roundi( realT a);
void out1( realT a);
void out2n( realT a, realT b);
void out3n( realT a, realT b, realT c);
...
int qh_rboxpoints(FILE* fout, FILE* ferr, char* rbox_command) {
  ...
  realT width=0.0, gap=0.0, radius= 0.0;
  realT coord[MAXdim], offset, meshm=3.0, meshn=4.0, meshr=5.0;
  realT *simplex= NULL, *simplexp;
  int nthroot, mult[MAXdim];
  realT norm, factor, randr, rangap, lensangle= 0, lensbase= 1;
  realT anglediff, angle, x, y, cube= 0.0, diamond= 0.0;
  realT box= qh_DEFAULTbox; /* scale all numbers before output */
  realT randmax= qh_RANDOMmax;
  ...
  *command= '\0';
  ...
  if (issimplex+issimplex2) {
    if (!(simplex= (realT*)qh_malloc( dim * (dim+1) * sizeof(realT)))) {
  ...
  /* ============= mesh distribution =============== */
  if (ismesh) {
    nthroot= (int)(pow((realT)numpoints, 1.0/dim) + 0.99999);
    for (k=dim; k--; )
      mult[k]= 0;
  ...
  /* ============= apex of 'Zn' distribution + gendim =============== */
    ...
    for (i=0; i < numpoints; i++) {
      ...
      /* ============= point of 'l' distribution =============== */
        ...
        coord[2]= 2.0*(realT)i/(realT)(numpoints-1) - 1.0;
