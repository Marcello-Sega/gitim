...
/*-<a                             href="qh-geom.htm#TOC"
  >-------------------------------</a><a name="weighted_pointdist">-</a>
  changed for ITIM ... add documentation here ....
  qh_pointdist( point1, point2, dim )
    return distance between two points
  ...
  notes:
    returns distance squared if 'dim' is negative
*/
coordT qh_weighted_pointdist(pointT *point1, pointT *point2, pointT * offset, int dim) {
  coordT dist, diff;
  int k;
  double weight;

  dist= 0.0;
  weight = *(qh_radii+((point1-offset)/dim) ) + *(qh_radii+((point2-offset)/dim));
  for (k= (dim > 0 ? dim : -dim); k--; ) {
    diff= *point1++ - *point2++;
    dist += diff * diff;
  }
  if (dim > 0)
    return(sqrt(dist)-weight);
  return dist;
} /* pointdist */
...
double * qh_setradii(double * radii,int nradii){

	/*qh_radii=(double*)realloc(qh_radii,nradii*sizeof(double));*/
	qh_radii=radii;
	fprintf(stderr,"&qh_radii= %p\n",qh_radii);
	return qh_radii;	
}
...
void qh_setdelaunay(int dim, int count, pointT *points) {
  ...
  coordT *coordp, coord; 
  realT paraboloid;
  if(qh_radii==NULL) exit(printf("qh_radii not allocated: use qh_setradii()\n"));
  trace0((qh ferr, 11, "qh_setdelaunay: project %d points to paraboloid for Delaunay triangulation\n", count));
  coordp= points;
  for (i=0; i < count; i++) {
    coord= *coordp++ ;
    paraboloid= 0*( coord*coord - (qh_radii[i]) * (qh_radii[i]));  
    for (k=dim-2; k--; ) {
      coord= *coordp++;
      paraboloid += coord*coord;
    }
