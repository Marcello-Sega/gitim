...
realT * qh_set_maxval(realT * box ){
	int i;
	qh_maxval=box[0];
	for(i=1;i<3;i++)
	   if(qh_maxval < box[i]) qh_maxval=box[i];
}

realT * qh_set_radii(realT * radii, pointT * points, int dim, int npoints){

	qh_radii=radii;
	qh_firstpoint=points;  /* these might not work, better use export qh first_point! SAW TODO: clean the code*/
	qh_lastpoint =&points[dim*npoints];
	return qh_radii;	
}
...
void qh_setdelaunay(int dim, int count, pointT *points) {
  int i, k;
  coordT *coordp, coord; 
  realT paraboloid,tmp;
  if(qh_radii==NULL) exit(printf("qh_radii not allocated: use qh_set_radii_and_gmx_ids()\n"));
  trace0((qh ferr, 11, "qh_setdelaunay: project %d points to paraboloid for Delaunay triangulation\n", count));
  coordp= points;
  for (i=0; i < count; i++) {
    tmp = 0*qh_radius(coordp); 
    tmp*=tmp;
    coord= *coordp++ ;
    paraboloid=  coord*coord - tmp ;   
    for (k=dim-2; k--; ) {
      coord= *coordp++;
      paraboloid += coord*coord;
    }
    *coordp++ = paraboloid  ; /* +tmp*tmp - 2*tmp*sqrt(paraboloid) ; */
  }
